...
class ConvBasic(nn.Module):
    def __init__(self, nIn, nOut, kernel=3, stride=1,
                 padding=1):
        #print('TESTLOCATOR',nIn, nOut, kernel, stride, padding)
        print(type(nOut))
...
class MSDNFirstLayer(nn.Module):
    def __init__(self, nIn, nOut, args):
        ...
        if args.data.startswith('cifar'):
            #print('TESTLOCATOR2',nOut, args.grFactor, args.grFactor[0])
            self.layers.append(ConvBasic(nIn, nOut * int(args.grFactor[0]),
                                         kernel=3, stride=1, padding=1))
        elif args.data == 'ImageNet':
            conv = nn.Sequential(
                    nn.Conv2d(nIn, nOut * int(args.grFactor[0], 7, 2, 3)),
                    nn.BatchNorm2d(nOut * int(args.grFactor[0])),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(3, 2, 1))
            self.layers.append(conv)

        nIn = nOut * int(args.grFactor[0])

        for i in range(1, args.nScales):
            self.layers.append(ConvBasic(nIn, nOut * int(args.grFactor[i]),
                                         kernel=3, stride=2, padding=1))
            nIn = nOut * int(args.grFactor[i])
...
class MSDNLayer(nn.Module):
    def __init__(self, nIn, nOut, args, inScales=None, outScales=None):
        ...
        if self.discard > 0:
            nIn1 = nIn * int(args.grFactor[self.offset - 1])
            nIn2 = nIn * int(args.grFactor[self.offset])
            _nOut = nOut * int(args.grFactor[self.offset])
            self.layers.append(ConvDownNormal(nIn1, nIn2, _nOut, args.bottleneck,
                                              args.bnFactor[self.offset - 1],
                                              args.bnFactor[self.offset]))
        else:
            self.layers.append(ConvNormal(nIn * int(args.grFactor[self.offset]),
                                          nOut * int(args.grFactor[self.offset]),
                                          args.bottleneck,
                                          args.bnFactor[self.offset]))

        for i in range(self.offset + 1, self.nScales):
            nIn1 = nIn * int(args.grFactor[i - 1])
            nIn2 = nIn * int(args.grFactor[i])
            _nOut = nOut * int(args.grFactor[i])
            self.layers.append(ConvDownNormal(nIn1, nIn2, _nOut, args.bottleneck,
                                              args.bnFactor[i - 1],
                                              args.bnFactor[i]))
    ...
    def forward(self, x):
        if self.discard > 0:
            ...
            print(x[0].shape)
            inp = torch.tensor(x[0]).unsqueeze_(0)
            #inp.unsqueeze_(0)
            print(inp.shape)
            for i in range(1, self.outScales):
                #print(x[i - 1].shape, x[i].shape)
                #print(x.shape)
                #inp.append([x[i - 1], x[i]])
                y = torch.cat((x[i - 1], x[i]), 0)

                inp = torch.cat((y, inp), 0)
...
class MSDNet(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.nBlocks):
            ...
            if args.data.startswith('cifar100'):
                ...
                self.classifier.append(
                    self._build_classifier_cifar(nIn * int(args.grFactor[-1]), 10))
            elif args.data == 'ImageNet':
                self.classifier.append(
                    self._build_classifier_imagenet(nIn * int(args.grFactor[-1]), 1000))
            else:
    ...
    def _build_transition(self, nIn, nOut, outScales, offset, args):
        ...
        for i in range(outScales):
            net.append(ConvBasic(nIn * int(args.grFactor[offset + i]),
                                 nOut * int(args.grFactor[offset + i]),
